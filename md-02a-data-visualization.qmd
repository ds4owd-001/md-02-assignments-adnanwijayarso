...
gapminder_summary_2007 <- gapminder|> 
  group_by(continent) |> 
  summarise(
    count = n(),
...
3.  Run the code contained in the code chunk and fix any errors.
    ...
    ```{r}
    ggplot(gapminder_summary_continent_year,
           mapping = aes(x = year,
                         y = lifeExp,
                         color = continent)) +
...
4.  The plot is identical to the plot created with `geom_col()`. Why? What does the `geom_bar()` function do? **The** `geom_bar()` **function would automatically count the variable we put in the x-axis to the y-axis**.
...
6.  Describe how the `geom_histogram()` function is similar to the `geom_bar()` function. **The** `geom_histogram()` **function also counts the variable we put on the x-axis.**

7.  What happens by adding the 'breaks' argument? Play around with the numbers inside of `seq()` to see what changes. **The 'breaks' argument defines the scale for our histogram. For example,** `breaks = seq(40, 85, 2)` **means that the x-axis would start at 40 and end at 85, and each column in the histogram has an interval of 2.**
...
4.  What is stored in the `country_colors` object? Find out by executing the object in the Console (type it to the Console and hit enter). Do the same again, but with a question mark `?country_colors`. **The** `country_colors` **object stored the color shade grouped by country. Adding question mark before the object assigns a color to each observations. There are 142 observations in our data, so executing it needs 142 colors and results in error because there is only 1 color provided.**
